...
\begin{itemize}
\item Assume constant supply of na\"ive cells is assumed \iffalse (to do: refs) \fi
...
\section{Analysis on two types of killing functions\label{sec:Analysis-on-two}}
\subsection{Model reduction}
We want to reduce the system (\ref{eq:3sp-model}) to two equations, on which we can conduct phase analysis. We do so by noting that the dynamics of peptides $P$ is at the molecular level and
...
patient data (Table \ref{tab:parameters}). Quasi-steady state approximation is often applied in cases like this by assuming the fast variable equilibrates instantly to its `quasi-steady state' and thus becomes a function of slow variables by setting its time derivative to be zero. In our case, we make quasi-steady state (QSS)
